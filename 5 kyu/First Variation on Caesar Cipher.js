...
function movingShift(s, shift) {
  ...
  // Begin block of cipher

  for (let i = 0; i < s.length; i++) {
    if (s.charCodeAt(i) >= 97 && s.charCodeAt(i) <= 122) {
      wordPosition = s.charCodeAt(i) - 97;

      if (wordPosition + i + shift >= 26) {
        sResult += wordsLowerCase[(wordPosition + i + shift) % 26];
        continue;
      }
      sResult += wordsLowerCase[wordPosition + i + shift];
    } else if (s.charCodeAt(i) >= 65 && s.charCodeAt(i) <= 90) {
      wordPosition = s.charCodeAt(i) - 65;
      if (wordPosition + i + shift >= 26) {
        sResult += wordsUpperCase[(wordPosition + i + shift) % 26];
        continue;
      }
      sResult += wordsUpperCase[wordPosition + i + shift];
    } else {
      sResult += s[i];
    }
  }
  ...
  // Begin wrapping cipher in 5 ambassadors

  let firstMates = Math.ceil(sResult.length / 5);
  let start = 0;
  let end = sResult.length;
  for (let i = 0; i < 5; i++) {
    if (end < firstMates) {
      arResult.push(sResult.slice(start, sResult.length));
      start = end;
    } else {
      arResult.push(sResult.slice(start, start + firstMates));
      end -= firstMates;
      start += firstMates;
    }
  }
...
function demovingShift(arr, shift) {
  ...
  // demoving cipher string

  for (let i = 0; i < s.length; i++) {
    if (s.charCodeAt(i) >= 97 && s.charCodeAt(i) <= 122) {
      wordPosition = s.charCodeAt(i) - 97;

      if (wordPosition - i - shift <= 0) {
        if (
          wordsLowerCase[26 - (-(wordPosition - i - shift) % 26)] == undefined
        ) {
          sResult += wordsLowerCase[0];
          continue;
        }
        sResult += wordsLowerCase[26 - (-(wordPosition - i - shift) % 26)];
      } else {
        sResult += wordsLowerCase[wordPosition - i - shift];
      }
    } else if (s.charCodeAt(i) >= 65 && s.charCodeAt(i) <= 90) {
      ...
      if (wordPosition - i - shift <= 0) {
        if (
          wordsUpperCase[26 - (-(wordPosition - i - shift) % 26)] == undefined
        ) {
          sResult += wordsUpperCase[0];
          continue;
        }
        sResult += wordsUpperCase[26 - (-(wordPosition - i - shift) % 26)];
      } else {
        sResult += wordsUpperCase[wordPosition - i - shift];
      }
    } else {
